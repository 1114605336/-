#include<stdio.h>  
#include<stdlib.h>  
/*  贪心法，从左边起依次扫描，每记录一个字母，就寻找从右边起最近的与它相同的字母，移动到回文的位置，记录步数
如果最近的与它相同的字母是它本身，说明它在字串里单独存在，可以根据单独存在的字母的个数以及字串的长度来判断impossible的情况
如果排除impossible的情况，则记录它到字串中间的步数，并不需要移动它。
*/
int main(){  
    int i,j,l,n,k,sum=0,flat=1,c=-1;
    char *a;
    scanf("%d",&n);
    a=(char *)malloc(n*sizeof(char));
    scanf("%s",a);
    j=n-1;  
    //利用贪心的思想，将每个遍历的字符找到后面与他相同的然后交换到正确的位置时所需的交换次数   
    for(i=0;i<j;i++){
        for(k=j;k>=i;k--){
            if(k==i){//说明没有找到与a[i]相同的字符   
                if(n%2==0||c!=-1){//如果n为偶数或者a[i]不是唯一一个单个无相同字符   
                    flat=0;
                    break;
                }
                c=1;//n为奇数，将第一个单个的字符a[i]移到中间位置所需的交换次数   
                sum=sum+n/2-i;
                break;
            }
            if(a[k]==a[i]){
                for(l=k;l<j;l++){  
                    a[l]=a[l+1];  
                }  
                a[j]=a[i];  
                sum=sum+j-k;  
                j--;  
                break;  
            }  
        }  
        if(flat==0){
            break;
        } 
    }  
    if(flat==0)
        printf("Impossible");
    else if(sum==0)
        printf("0");
    else 
        printf("%d\n",sum);
    return 0;  
}
